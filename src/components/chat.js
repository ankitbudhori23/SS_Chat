...
import {
  ...
  incrementUnread,
  resetUnread,
} from "@/state/chatList/chatListSlice";
import {
  setMessages,
  addMessage,
  addOlderMessages,
...
export default function Chat() {
  ...
  const filteredChats =
    filter === "unread"
      ? nameFilteredChats.filter((chat) => chat.client_unread > 0)
      : nameFilteredChats;
  ...
  useEffect(() => {
    ...
    const client = mqtt.connect(process.env.NEXT_PUBLIC_CHAT_BROKER, {
  ...
  const handleChatChange = (chat) => {
    ...
    if (
      selectedChatState?.chat_group_id &&
      selectedChatState.chat_group_id !== chat.chat_group_id
    ) {
    ...
    const newTopic = !chat.is_group
      ? `chat/${[username, chat.receiver_id].sort().join("-")}`
      : `chat/${chat.chat_group_id}`;
  ...
  const chatters = filteredChats
    ? filteredChats.map((chat, index) => {
        ...
        return (
          <a
            key={index}
            className={`${styles.chatter_cn} ${
              isActive ? styles.active_chatter : ""
            }`}
            onClick={() => handleChatChange(chat)}
          >
            <img
              src={`${chat.is_group ? "/group.png" : "/profile-gray.png"}`}
              className={`${styles.chatter_img} ${
                chat.is_group ? styles.group_image : ""
              }`}
            ></img>
            <div className={styles.chatter_info}>
              ...
              {chat.client_unread > 0 && (
                <div className={styles.chatter_unread}>
                  {chat.client_unread}
                </div>
              )}
            </div>
    ...
    <div className={styles.chat_page}>
      <div className={styles.side_bar}>
        <div className={styles.side_bar_top}>
          {/* <a
            href="/"
            className={`${styles.side_bar_icon_img} ${styles.chat_img}`}
          ></a>
        ...
        {/* <div className={styles.side_bar_bottom}>
          <div
            className={`${styles.side_bar_icon_img} ${styles.settings_img}`}
          ></div>
          <a
            href="dashboard"
            className={`${styles.side_bar_icon_img} ${styles.profile_img}`}
          ></a>
        </div> */}
      </div>
      ...
      {/* Chat List */}
      <div
        className={`${styles.chat_bar} ${
          showChatList ? styles.show_mobile : ""
        }`}
      >
        <div className={styles.chat_bar_top}>
          ...
          <div className={styles.tags_list}>
            ...
            <button
              className={`${styles.tag} ${
                filter === "unread" ? styles.tag_active : ""
              }`}
        ...
        <div className={styles.chat_list}>{chatters}</div>
      </div>
      ...
      {showMediaOverlay && (
        <div className={styles.media_overlay}>
          <div className={styles.media_overlay_top}>
            <button
              className={styles.media_overlay_close}
              onClick={() => {
                setShowMediaOverlay(false);
                setMediaType(null);
                setMediaURL(null);
          ...
          <div className={styles.media_overlay_bottom}></div>
        </div>
      )}
    </div>
  );
