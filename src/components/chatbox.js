...
export default function ChatBox({ topicRef, clientRef, fetchOlderMessages, setShowMediaOverlay, setMediaURL, setMediaType }){
    ...
    const currentChat = chatList.find(c => c.chat_group_id === chatState?.chat_group_id);
    ...
    useEffect(() => {
        const getMembers = async() => {
            const memberList = await fetchGroupMembers(accessToken, chatState.chat_group_id)
        ...
        const handleLoadingMessages = async () => {
            if (container.scrollTop == 0 && !loadingOlderRef.current) {
                ...
                requestAnimationFrame(() => {
                    ...
                    container.scrollTop = newScrollHeight - prevScrollHeight + prevScrollFromTop
        ...
        <div className={styles.chat_main} >
            {
                ...
                ?
                <div className={styles.chat_main_placeholder}>Select a group to start chatting with your teacher.</div>
                :
                <div className={styles.curr_chat_main_cn}>
                    <div className={styles.curr_chat_head_cn}>
                        <button className={styles.curr_chat_head} onClick={() => setShowGroupDesc(true)}>
                            <div className={styles.curr_chat_img}>
                                <img src={`${chatState.is_group ? "/group.png"  : "/profile-gray.png" }`}></img>
                            </div>
                            <div className={styles.curr_chat_name}>
                                <h2>
                                    {`${capitalizeFirstWord(chatName[0])} ${capitalizeFirstWord(chatName[1])}`}
                                </h2>
                                <div className={styles.curr_chat_status}>{`${chatState.is_group ? "Group Description" : "Bio"}`}</div>
                            </div>
                        </button>
                    </div>
                    <div className={styles.curr_chat_main} ref={chatRef}>
                        {   
                            ...
                            messagesState.map((msg, index) => {
                                ...
                                const showDateDivider = index == 0 || new Date(msg.created_at).toDateString() !== new Date(prevMsg?.created_at).toDateString()
                                return (
                                <React.Fragment key={`${msg.id}-${index}`}>
                                    ...
                                    {/* Chat Messages */}
                                    <div className={`${styles.message} ${msg.sender_id === userState?.user_id ? styles.curruser : ""}`}>
                                        {
                                            (chatState.is_group == 1 &&  msg.sender_id !== userState?.user_id) &&<div className={styles.sender_name}>{groupMembersMap.get(msg?.sender_id)}</div>
                                        }
                                        {
                                            msg.media_url 
                                            && 
                                            (<>
                                                ...
                                                {msg.message_type === "image" && (() => {
                                                    ...
                                                    return urls.map((url, idx) => (
                                                        <button 
                                                            key={idx} 
                                                            onClick={() => handleMedia(url, msg.message_type)} 
                                                            className={styles.message_media_cn}
                                                        >
                                                            <img 
                                                                src={url} 
                                                                alt="image" 
                                                                className={`${styles.message_media} ${msg.receiver_id === chatState.receiver_id ? styles.curruser : ""}`} 
                                                            />
                                                        </button>
                                                    ));
                                                })()
                                                ...
                                                {msg.message_type === "video" && (
                                                    <button onClick={() => handleMedia(msg.media_url, msg.message_type)} className={styles.message_media_cn}>
                                                        <video src={msg.media_url} controls className={`${styles.message_media} ${msg.receiver_id === chatState.receiver_id ? styles.curruser : ""}`} />
                                                    </button>
                                                )}

                                                {/* Document */}
                                                {msg.message_type === "document" && (
                                                    <button onClick={() => handleMedia(msg.media_url, msg.message_type)} className={`${styles.message_media} ${msg.receiver_id === chatState.receiver_id ? styles.curruser : ""}`} >
                                        ...
                                        <div className={styles.message_time}>{timeString(msg?.created_at)}</div>
                                    </div>
...

            {
                ...
                &&
                <div className={styles.modal_overlay} onClick={() => {setShowGroupDesc(false)}}>
                    <div className={`${styles.modal} ${showGroupDesc ? styles.modal_show : ""}`} onClick={(e) => {
                        e.stopPropagation()
                    }}>
                        <button className={styles.modal_close}  onClick={() => setShowGroupDesc(false)}>
                            &#10005;
                        </button>
                        ...
                        <div className={styles.modal_head}>
                            ...
                            <div className={styles.modal_name}>
                                ...
                                <div className={styles.modal_status}>{`${chatState.is_group ? "Group Description" : "Bio"}`}</div>
                            </div>
                        </div>

                        {
                            ...
                            <div className={styles.members}>
                                ...
                                {groupMembers.map((member, index) => {
                                    ...
                                    const isCurrUser = name === capitalizeFirstWord(userState?.fname) + " " + capitalizeFirstWord(userState?.lname)
                                    return (
                                        <p key={index}>{`${member.name} ${isCurrUser ? "(You)": ""}`}</p>
                                    )
                                })}
                            </div>
                        }
