...
export default function TypeBox({ topicRef, clientRef, groupMembers }) {
      ...
      clientRef.current.publish(
        ...
        JSON.stringify({
          ...
          data: {
            chat_group_id: `${chat.chat_group_id}`,
            display_name:
              `${userState.fname} ${userState.lname}` || "New message",
            is_group: "0",
            receiver_id: `${chat.receiver_id}`,
            screen: "Chat",
          },
  ...
  useEffect(() => {
    ...
    const singleUploadReady = Boolean(filePreview && uploadedFile);
    const galleryUploadReady =
      imagePreviews.length > 0 && uploadedImages.length > 0;

    if (singleUploadReady || galleryUploadReady) {
      // Auto-submit media messages as soon as uploads finish.
      sendMessage();
    }
  }, [
    fileUploading,
  ...
  return (
    <div className={styles.type_box} ref={typeBoxRef}>
      <div
        className={`${styles.type_box_util_upload} ${
          typeBoxUtil ? styles.show : ""
        }`}
      >
        <div
          className={`${styles.upload_optn} ${typeBoxUtil ? styles.show : ""}`}
        >
      ...
      {filePreview && (
        <div className={styles.file_preview}>
          {filePreview.type === "video" && (
            <video
              src={filePreview.url}
              className={styles.file_preview_video}
              muted
            />
          )}
      ...
      <button
        className={styles.type_box_util}
        onClick={() => setTypeBoxUtil((prev) => !prev)}
      ></button>
      <textarea
